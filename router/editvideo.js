...
router.post('/editvideobylink' , async (req, res) => {
    console.log('connection of editvideobylink done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to editvideobylink...");
      var newHeaders = [];
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Video.findOne({ VideoId: jasonn.VideoId }).then(async (videoexist) => {
                    ...
                    if (videoexist) {
                        console.log("l1");
                        const total_products = await Video.deleteOne({VideoId: jasonn.VideoId});
