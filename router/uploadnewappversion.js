...
router.post('/uploadAppUpdate' , async (req, res) => {
    console.log('connection of appupdate done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to appupdate");
      var newHeaders = [];
      ...
      if (refer == true) {
        //   console.log(User)
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            //   console.log(size_v);
              console.log("user");
              if(user.UserRole=="Admin") { //in bytes
                console.log("size is perfecttttttttttttttttttt");
                AppUpdate.findOne({ AppVersion: jasonn.AppVersion }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (!videoexist ) {
                        console.log("l1");
                        const appupdate = new AppUpdate(jasonn);
                            const videoinfo = await appupdate.save()
                            console.log(videoinfo);
                            res.status(201).send("Version Upload Successful!");
                    } else if(videoexist!=null){
                        res.status(202).send("App Version ALready Exist!");
                    }
