...
router.post('/AddNewMusicCatagory' , async (req, res) => {
    console.log('connection of addmusiccata done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to addmusiccatagory");
      var newHeaders = [];
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            //   console.log(size_v);
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                AddMusicCatagory.findOne({ title: jasonn.title }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (!videoexist ) {
                        console.log("l1");
                            const newmusiccatagory = new AddMusicCatagory(jasonn
                            );
                            const videoinfo = await newmusiccatagory.save()
                            console.log(videoinfo);
                            const all_products = await AddMusicCatagory.aggregate([
                                // {$unwind: '$products'},
                                {$match: {}},]
                                ).sort({"index":1});
                            res.status(201).send(all_products);
                    } else if(videoexist!=null){
                        console.log("catagory.tit");
                        res.status(202).send("Music Catagory ALready Exist!");
                    }
