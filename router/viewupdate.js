...
router.post('/viewupdate' , async (req, res) => {
    console.log('connection of viewupdate done');
    ...
    //validating jwttoken
    try {
      console.log("enteringggg to viewupdateviewupdatevv");
      var newHeaders = [];
                ...
                Video.aggregate([
                    ...
                    if (userExist1.length==0) {
                        ...
                        const mdone = await Video.updateOne({ VideoId: videoid },
                            { "$set": { "no_views": videoh.no_views+1 }, },{upsert:false,strict:false});
                        ...
                        // var report=0;
                        // var watchtim=0;
                        // for(var i =0;i<videoh.viewers.length;i++) {
                        //     if(videoh.viewers[i].like=="Yes") {
                        //         like=+1;


                        //     }else if(videoh.viewers[i].like=="No") {
                        //         dislike=+1;
                        //     };
                        //     watchtim=+videoh.viewers[i].watchtime;
                        //     if(videoh.viewers[i].report==true) {
                        //         report=+1;
                        //     }
                        // };
                        // const mdone = await Video.updateOne({VideoId: videoid },
                        //     { "$set": { 
                        //         "no_views": videoh.viewers.length+1,
                        //         "watch_time": watchtim,
                        //         "no_Reports": report,
                        //         "video_like": like,
                        ...
                        //     { $unwind: '$viewers'},
                        //     { $match: { $and: [,
                        //      { "viewers.like":  jasonn.like },
                        //      { "viewers.report":  jasonn.report }] } }]).then(async (userExist2) => {
