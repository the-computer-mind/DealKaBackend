...
router.post('/approvecourse' , async (req, res) => {
    console.log('connection of approvecoursessssssssssss done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to approvecoursessssssssssss");
      var newHeaders = [];
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Course.findOne({ CourseId: jasonn.CourseId }).then(async (Courseexist) => {
                    ...
                    if (Courseexist ) {
                        console.log("l1");
                        const mdone = await Course.updateOne({ CourseId: jasonn.CourseId },
                            { "$set": { "Approved": approve } },{upsert:false,strict:false});
                        console.log(mdone);
                        const Coursehh = await Course.findOne({CourseId: jasonn.CourseId});

                        
                        res.status(201).send(Coursehh);
                    } else if(!Courseexist){
