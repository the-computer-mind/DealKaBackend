...
const Banner = require("../model/banners_model");
app.use(express.json());
const { json } = require('express');
...
router.post('/AddNewBanner' , async (req, res) => {
    console.log('connection of AddNewBanner done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to AddNewBanner");
      var newHeaders = [];
      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            var jasonn= req.body;
              ...
              if(user.UserRole=="Admin" || user.UserRole=="BannerModarator" ) { //in bytes
                console.log("size is perfecttttttttttttttttttt");
                Banner.findOne({ OuterBannerId: jasonn.OuterBannerId }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (!videoexist ) {
                        console.log("l1");
                        const banner = new Banner(jasonn
                            );
                            const videoinfo = await banner.save()
                            console.log(videoinfo);
                            res.status(201).send("Successfully saved!");
                    } else if(videoexist!=null){
                        console.log("videoexist.tit");
                        const total_products = await Banner.deleteOne({ OuterBannerId: jasonn.OuterBannerId });
                        const banner = new Banner(jasonn
                            );
                            const videoinfo = await banner.save()
                            console.log(videoinfo);
                            res.status(201).send("Successfully Delete and saved!");
                    }
