...
try{router.post('/getallevents',  async (req, res ) => {

    try {
    console.log("enteringggg to getallevents");
    var newHeaders = [];
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            const total_products = await Event.aggregate([
                // {$unwind: '$products'},
                {$match: {}},]
                )
            console.log("okkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk"+total_products.length);
            const all_products = await Event.aggregate([
                // {$unwind: '$products'},
                {$match: {}},]
                ).sort({_id: -1 });
            ...
            console.log(searchquery);
                ...
                const all_products = await Event.find(
