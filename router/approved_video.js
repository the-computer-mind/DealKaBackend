...
router.post('/approvevideo' , async (req, res) => {
    console.log('connection of approvevideo done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to approvevideo");
      var newHeaders = [];
      newHeaders = req.header("authorization").split(",");
      console.log(req.header("authorization")+"hiii");
      // console.log(req.header("size") + "hiii");
      var approve = req.header("Approve");
      
      //var videoid = req.header("videoId");
      // const productata = JSON.parse(req.body);
      console.log(approve); //this is the productmodel coming from flutter
      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Video.findOne({ VideoId: jasonn.VideoId }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (videoexist ) {
                        console.log("l1");
                        const mdone = await Video.updateOne({ VideoId: jasonn.VideoId },
                            { "$set": { "Approved": approve } },{upsert:false,strict:false});
                        console.log(mdone);
                        const videohh = await Video.findOne({VideoId: jasonn.VideoId});
