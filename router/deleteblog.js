...
router.post('/deleteblog' , async (req, res) => {
    console.log('connection of deleteblogg done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to deleteeeeeeeblogh");
      var newHeaders = [];
      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Blog.findOne({ BlogId: jasonn.BlogId }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (videoexist ) {
                        console.log("l1");
                        const total_products = await Blog.deleteOne({BlogId: jasonn.BlogId});
                        console.log(total_products);
                        res.status(201).send("deleted blog!");
                    } else if(!videoexist){
                        res.status(202).send("Blog Not Exist!");
                    }
