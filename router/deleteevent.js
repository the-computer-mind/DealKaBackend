...
router.post('/deleteevent' , async (req, res) => {
    console.log('connection of deleteevent done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to deleteeventeeevent");
      var newHeaders = [];
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            //   console.log(size_v);
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Event.findOne({ EventId: jasonn.EventId }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (videoexist ) {
                        console.log("l1");
                        const total_products = await Event.deleteOne({EventId: jasonn.EventId});
                        console.log(total_products);
                        res.status(201).send("videoinfo");
                    } else if(!videoexist){
                        res.status(202).send("Event Not Exist!");
                    }
