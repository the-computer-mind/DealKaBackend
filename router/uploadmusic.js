...
router.post('/uploadmusiclink' , async (req, res) => {
    console.log('connection of uploadmusiclink done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to uploadmusiclink");
      var newHeaders = [];
      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Music.findOne({ music_name: jasonn.music_name }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (!videoexist) {
                        console.log("l1");
                        const music = new Music(jasonn
                            );
                            const musicinfo = await music.save()
                            console.log(musicinfo);
                            res.status(201).send("musicinfo");
                    } else if(videoexist!=null){
                        console.log(videoexist.music_name);
