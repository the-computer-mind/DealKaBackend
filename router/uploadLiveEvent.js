...
router.post('/uploadliveevent' , async (req, res) => {
    console.log('connection of uploadliveevent done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to uploadliveevent");
      var newHeaders = [];
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            //   console.log(size_v);
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Live.findOne({ EventId: jasonn.EventId }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (!videoexist ) {
                        console.log("l1");
                        const live = new Live(jasonn
                            );
                            const videoinfo = await live.save()
                            console.log(videoinfo);
                            res.status(201).send("upload live success!");
                    } else if(videoexist!=null){
                        console.log("videoexist.tit");
                        res.status(202).send("Live ALready Exist!");
                    }
