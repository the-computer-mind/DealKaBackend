...
try{router.post('/getallshortvideos',  async (req, res ) => {

    try {
    console.log("enteringggg to getallshortvideos");
    var newHeaders = [];
    ...
    // var size = req.header("size");
    var size = 20;
    
    // const productata = JSON.parse(req.body);
    console.log(size); //this is the productmodel coming from flutter
    ...
    const obj = [];
    var skip = (parseInt(page)*size-size);
    console.log(skip);
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            const all_products = await Video.aggregate([
                ...
                ).sort({"Algorithmpush":-1, _id: -1 }).skip(skip).limit(20);
            ...
            console.log(searchquery);
                ...
                const all_products = await Video.find(
                    ...
                    ).sort({"Algorithmpush":-1}).skip(skip).limit(20);
                console.log(all_products);
