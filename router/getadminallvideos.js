...
try{router.post('/getadminallvideos',  async (req, res ) => {

    try {
    console.log("enteringggg to getadminallvideos");
    var newHeaders = [];
    ...
    console.log("ok back from jwtauth to adminpanelallvideos");
    const obj = [];
    ...
    console.log(user);
    if (refer == true && user.UserRole=="Admin") {
         //console.log(user)
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            console.log('under getadminvido');
            var count;
            console.log( search);
            if(search=="false"){
            ...
            console.log("loppppppppppploppppppp"+total_products.length);
            const all_Videos = await Video.aggregate([
                // {$unwind: '$products'},
                {$match: {}},]
                ).sort({"Algorithmpush":-1,"Approved":1}).skip(skip).limit(no_of_videos_sendig);
            const all_Blogs = await Blog.aggregate([
                    // {$unwind: '$products'},
                    {$match: {}},]
                    );
            const all_Course = await Course.aggregate([
                        // {$unwind: '$products'},
                        {$match: {}},]
                        ).sort({"Algorithmpush":-1,"Approved":1}).skip(skip).limit(no_of_videos_sendig);
            const all_products=[all_Videos,all_Blogs,all_Course];
            ...
            console.log(searchquery);
                ...
                const all_products = await Video.find(
                    ...
                    ).sort({"Video_upload_date":1}).skip(skip).limit(no_of_videos_sendig);
                // console.log(all_products);
                res.setHeader('total_products',total_products.length);
                res.status(201).send(all_products);}
            }
