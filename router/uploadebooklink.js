...
router.post('/uploadebooklink' , async (req, res) => {
    console.log('connection of uploadebooklink done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to uploadebooklink");
      var newHeaders = [];
      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              var jasonn= req.body;
              var size_v = (jasonn.ebook_size);
              console.log(size_v);
              console.log("user");
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                console.log("size is perfecttttttttttttttttttt");
                Ebook.findOne({ ebook_name: jasonn.ebook_name }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (!videoexist) {
                        console.log("l1");
                        const ebook = new Ebook(jasonn
                            );
                            const ebookinfo = await ebook.save()
                            console.log(ebookinfo);
                            res.status(201).send("ebookinfo");
                    } else if(videoexist!=null){
                        console.log(videoexist.ebook_name);
