...
try{router.post('/getallbanners',  async (req, res ) => {

    try {
    console.log("enteringggg to getallbanners");
    var newHeaders = [];
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            console.log('under getall banner');
            // var count;
            // console.log( search);
            
                
            const total_products = await Banner.aggregate([
                // {$unwind: '$products'},
                {$match: {}},]
                )
            console.log("loppppppppppploppppppp"+total_products.length);
            const all_products = await Banner.aggregate([
                // {$unwind: '$products'},
                {$match: {}},]
                )
