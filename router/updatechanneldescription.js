...
try{
    ...
    router.post('/UpdateChanneldescription',  async (req, res ) => {

    try {
    console.log("enteringggg to UpdateChanneldescription");
    var newHeaders = [];
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            if(search=="false"){
                const channeldescrip = await Channel.updateOne({name:user.name },
                    { "$set": { "ChannelProfilePicLink": req.body.profilelink },
                 },

                    {upsert:false,strict:false});
                const channeldescrip1 = await Channel.updateOne({name:user.name },
                        { 
                        "$set": { "ChannelDescription": req.body.channeldescription }
                     },
                     
                        {upsert:false,strict:false});
                const channeldescrip2 = await Channel.updateOne({name:user.name },
                            {
                            "$set": { "ChannelBannerVideo": [JSON.parse(req.body.bannervideo)] },
                         },)
                ...
                console.log(channeldescrip,channeldescrip1,channeldescrip2);
                console.log("line 126 of productadd");
            res.status(201).send(chanel);
