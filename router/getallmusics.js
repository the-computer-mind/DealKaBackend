...
try{router.post('/getallmusics',  async (req, res ) => {

    try {
    console.log("enteringggg to getallmusics");
    var newHeaders = [];
    ...
    var Catagory = req.header("Catagory");
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            console.log('under getallmusicssssssssssss');
            var count;
            console.log( search);
            if(search=="false"){
                
            const total_products = await Music.aggregate([
                // {$unwind: '$products'},
                {$match: {'Approved': false}},]
                )
            console.log("okkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk"+total_products.length);
            const all_products = await Music.aggregate([
                // {$unwind: '$products'},
                {$match: {'Approved': false}},]
            ...
            if(Catagory=="") {
                ...
                const all_products = await Music.find(
                ...
                console.log("tata"+total_products.length);
                const all_products = await Music.find(
                    {"Music_Type" : Catagory},).sort({"Algorithmpush":-1, _id: -1 })
                res.setHeader('total_products',total_products.length);
                res.status(201).send(all_products);
            }
