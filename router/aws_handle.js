...
router.post('/s3url' , async (req, res) => {
    console.log('connection of s3 done');
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to getallproducts");
      var newHeaders = [];
      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              var size_v = (req.body.video_size);
              console.log(typeof(size_v));
              console.log(user);
              if(size_v<487241590) { //in bytes
                console.log("size is perfecttttttttttttttttttt");
                var url = await generateUploadURL(user.name);
