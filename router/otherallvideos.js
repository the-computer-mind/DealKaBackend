...
try{router.post('/OthersAllVideos',  async (req, res ) => {

    try {
    console.log("enteringggg to OthersAllVideos");
    var newHeaders = [];
    ...
    var othername = req.header("otherprofilename");
    ...
    console.log( process.env.jwt_token);
    //const user = await User.findOne({username : othername});
    if (refer == true) {
        //console.log(user)
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            const channeldescrip2= await Channel.findOne(
                {name : othername},);
            const all_products = [videos,channeldescrip2,blogs,courses];
            const total_products = all_products.length
            console.log(all_products);
            console.log(channeldescrip2);
            console.log("kiiiiiiiiiiiioooo");
            console.log(blogs);
            


            res.setHeader('total_products',total_products);
            ...
            console.log(searchquery);
                ...
                const all_products = await Video.find(
