...
router.post('/addphototoplace' , async (req, res) => {
    console.log('connection of addphototoplace done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to addphototoplace");
      var newHeaders = [];
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            //   console.log(size_v);
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Place.findOne({ PlaceId: jasonn.PlaceId }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (videoexist ) {
                        //console.log(videoexist);
                        // const videoh = await Place.findOne({PlaceId: jasonn.PlaceId});
                        // const newlist = jasonn.photogalary.concat(videoexist.photogalary);
                        // console.log(newlist);
                        // jasonn.photogalary=newlist;
                        console.log("now see update valu");
                        console.log(jasonn.photogalary);
                        const done = await Place.deleteOne({ PlaceId: jasonn.PlaceId });
                        const place = new Place(jasonn
                            );
                        const videoinfo = await place.save()
                        console.log(videoinfo);
                        console.log(done);
                        const all_products = await Place.aggregate([
                            // {$unwind: '$products'},
                            {$match: {}},]
                            ).sort({"index":-1});
                        ...
                        console.log("videoexist.tit");
                        res.status(202).send("Video Not Exist!");
                    }
