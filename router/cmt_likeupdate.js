...
router.post('/cmtlikeupdate' , async (req, res) => {
    console.log('connection of cmtlikeupdate done');
    ...
    //validating jwttoken
    try {
      console.log("enteringggg to cmtlikeupdatelikeupdatevv");
      var newHeaders = [];
      ...
      var cmtid = req.header("CommentId");
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Comment.findOne({ typeid: videoid }).then(async (userExist1) => { 
                    ...
                    if (userExist1.length!=0) {
                        ...
                        if(like=="like"){
                            const cmt_user =  await Comment.findOne({"comments":{"$elemMatch": {
                                "commentId": cmtid,
                                "comment_like": {"$elemMatch": {
                                    "cmt_like_userName": username,
                                }}}}});
                            console.log(cmt_user);
                            if(cmt_user==null){
                            const mdone = await Comment.updateOne({ typeid: videoid  },
                            { "$push": {[`comments.${index}.comment_like`] :jasonn  }, },{upsert:false,strict:false});
                            console.log(mdone);}
                        
                        }else if(like=="dislike"){
                            const cmt_user =  await Comment.findOne({'comments.comment_dislike.cmt_dislike_userName':username});
                            console.log(cmt_user);
                            if(cmt_user==null){
                            const mdone = await Comment.updateOne({ typeid: videoid  },
                            { "$push": {[`comments.${index}.comment_dislike`] :jasonn  }, },{upsert:false,strict:false});
                            console.log(mdone);}
                        ...
                        const videohh = await Comment.findOne({typeid: videoid});
                        ...
                        res.status(202).send("Already Liked/disliked video");
                        // if(userExist1[0].viewers.like=='null' && userExist1[0].viewers.like!=jasonn.like ) {
                        //     console.log("if userex.like==null");
                        //     if(jasonn.like=='null') {
                        //         console.log("if like==like");
                        //         res.status(202).send("Already Liked/disliked video");
                        //     }else if(jasonn.like=='like') {
                        //         console.log("json.like==like");
                        //         console.log(userExist1[0].viewers.viewerusername);
                                
                        //         const done2 = await Video.updateOne({ VideoId: videoid },
                        //             { "$pull": { "viewers":{"viewerusername":userExist1[0].viewers.viewerusername}}, },{ safe: true, multi:true });
                        //         console.log(done2);
                        ...
                        //                 { "$set": { "video_like": videoh.video_like+1 }, },{upsert:false,strict:false});
                        //         const videohh = await Video.findOne({VideoId: videoid});
    
                        //         res.status(201).send(videohh);
                        //     } else if(jasonn.like='dislike') {
                        //         console.log("json.like==dislike");
                        //         console.log(userExist1[0].viewers.viewerusername);
                        ...
                        // }else if (userExist1[0].viewers.like==jasonn.like) {
                        //     console.log("if like==like");
                        //     res.status(202).send("Already Liked video");
                        // } else if (userExist1[0].viewers.like=='dislike' && userExist1[0].viewers.like!=jasonn.like) {
                        //     console.log("if userex.like==dislike");
                        //     if(jasonn.like=='null') {
                        //         console.log("json.like==null");
                        //         console.log(userExist1[0].viewers.viewerusername);
                        ...
                        //         console.log(videoh.no_views);
                        //         const mdone = await Video.updateOne({ VideoId: videoid },
                        //             { "$set": { "video_dislike": videoh.video_dislike-1 , "video_like": videoh.video_like+1}  },{upsert:false,strict:false});
                        //         console.log(mdone);
                        //         const videohh = await Video.findOne({VideoId: videoid});
                        ...
                        // }else if(userExist1[0].viewers.like=='like' && userExist1[0].viewers.like!=jasonn.like) {
