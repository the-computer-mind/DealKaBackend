...
router.post('/uploadevent' , async (req, res) => {
    console.log('connection of uploadevent done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to uploadevent");
      var newHeaders = [];
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            //   console.log(size_v);
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Event.findOne({ title: jasonn.title }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (!videoexist ) {
                        console.log("l1");
                        const event = new Event(jasonn
                            );
                            const videoinfo = await event.save()
                            console.log(videoinfo);
                            res.status(201).send("videoinfo");
                    } else if(videoexist!=null){
                        console.log("videoexist.tit");
                        res.status(202).send("Event ALready Exist!");
                    }
