...
try{router.post('/getallblogs',  async (req, res ) => {

    try {
    console.log("enteringggg to getallblogs");
    var newHeaders = [];
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            console.log('under under get blogs....');
            var count;
            console.log( search);
            if(search=="false"){
                
            const total_products = await Blog.aggregate([
                // {$unwind: '$products'},
                {$match: {'Approved': false}},]
                )

            console.log("okkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk"+total_products.length);
            const all_products = await Blog.aggregate([
                // {$unwind: '$products'},
                {$match: {'Approved': false}},]
            ...
            console.log(searchquery);
                ...
                const all_products =  await Blog.aggregate([
                    // {$unwind: '$products'},
                    {$match: {"tags" : {$regex : '.*' + searchquery + '.*'},'Approved': false}},
