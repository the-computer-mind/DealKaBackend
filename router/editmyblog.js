      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Blog.findOne({ BlogId: blogId }).then(async (blogexsit) => {
                    if (blogexsit) {
                        try{console.log("l1");
                        console.log(jasonn.unfinished);
                        const total_products = await Blog.deleteOne({ BlogId: blogId});
                        console.log(total_products);
                        const blog = new Blog(jasonn
                            );
                        const bloginfo = await blog.save();
                        const bloged = await Blog.findOne({ BlogId: blogId });
                        console.log(bloginfo);
                        console.log(bloged);
                        res.status(201).send(bloged);
                        }catch(err) {
                                console.log(err);
