...
router.post('/deleteliveevent' , async (req, res) => {
    console.log('connection of deleteliveevent done');
    console.log(req.body);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to deleteliveevent");
      var newHeaders = [];
            ...
            //   console.log(size_v);
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Live.findOne({ EventId: jasonn.EventId }).then(async (videoexist) => {
                    ...
                    if (videoexist ) {
                        console.log("l1");
                        const total_products = await Live.deleteOne({EventId: jasonn.EventId});
                        console.log(total_products);
                        res.status(201).send("videoinfo");
                    } else if(!videoexist){
                        res.status(202).send("Live Not Exist!");
                    }
