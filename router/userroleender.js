...
try{router.post('/userrole',  async (req, res ) => {

    try {
    console.log("enteringggg to userrole");
    var newHeaders = [];
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            const total_products = await User.findOne({name: username});
            // cluster.worker.kill();
            // console.log("killed worker ");
            if(total_products!=null){console.log("okkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk"+total_products);
            
            res.setHeader('userrole',total_products.UserRole);
            res.status(201).send(total_products.UserStatus);}
            else {
                res.setHeader('userrole',null);
