...
router.post('/uploadvideolink' , async (req, res) => {
    console.log('connection of uploadvideolink done');
    console.log(req.body);
    console.log(req.body.viewers);
    // res.status(201).send("url");
    //validating jwttoken
    try {
      console.log("enteringggg to videouploadlink");
      var newHeaders = [];
      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Video.findOne({ video_name_inphone: jasonn.video_name_inphone }).then(async (videoexist) => {
                    console.log(videoexist);
                    if (!videoexist || videoexist.video_name_inphone=="upload_from_link") {
                        console.log("l1");
                        const video = new Video(jasonn
                            );
                            const videoinfo = await video.save()
                            console.log(videoinfo);
                            res.status(201).send("videoinfo");
                    } else if(videoexist!=null){
                        console.log(videoexist.video_name_inphone);
                        // res.status(202).send("Video ALready Exist!");
                    }
