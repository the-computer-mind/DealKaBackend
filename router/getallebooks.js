...
try{router.post('/getallebooks',  async (req, res ) => {

    try {
    console.log("enteringggg to getallebooks");
    var newHeaders = [];
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            const total_products = await Ebook.aggregate([
                // {$unwind: '$products'},
                {$match: {'Approved': false}},]
                )
            console.log("okkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk"+total_products.length);
            const all_products = await Ebook.aggregate([
                // {$unwind: '$products'},
                {$match: {'Approved': false}},]
            ...
            console.log(searchquery);
                ...
                const all_products = await Ebook.find(
