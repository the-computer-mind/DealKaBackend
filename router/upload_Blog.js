...
router.post('/uploadblog' , async (req, res) => {
    ...
    //validating jwttoken
    try {
      console.log("enteringggg to blogupload");
      var newHeaders = [];
      ...
      if (refer == true) {
          ...
          if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
              ...
              var jasonn= req.body;
              var size_v = (jasonn.Blog_size);
              console.log(size_v);
              console.log("user");
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                console.log(jasonn);
                
                Blog.findOne({ BlogId: jasonn.BlogId }).then(async (blogexsit) => {
                    if (!blogexsit) {
                        try{console.log("l1");
                        console.log(jasonn.unfinished);
                        const blog = new Blog(jasonn
                            );
                        ...
                        res.status(201).send("bloginfo");
                                ...
                                console.log(err);
                                res.status(202).send("something wrong");
                            }
                    }
                    else if(videoexist){
                        const blo = await Blog.findOne({ BlogId: jasonn.BlogId });
                        console.log("lllllllllllllllllllll");

                        if(blo.unfinished==true){
                            const mdone = await Blog.deleteOne({ BlogId: jasonn.BlogId });
                            console.log(mdone);
                            res.status(205).send("delete wrong")
