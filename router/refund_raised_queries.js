...
try{router.post('/Moderator_Refund_Queries',  async (req, res ) => {

    try {
    console.log("enteringggg to refundcustomer");
    var newHeaders = [];
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            ...
            const all_products = await Customer.aggregate([
                // {$unwind: '$products'},
                {$match: {'isRefundQuaery_raised': true}},
                {$match: {'transaction_closed': false}},
                {$match: { 'isproduct_dispatch': {$ne :"Cancel"}}},
                {$match: {'isPayaco_Moderator_Assign': false}},
    ...
    // if(!email || !password){
    //     return res.send.json({error: "plz usern all field"});
    // };
