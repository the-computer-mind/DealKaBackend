...
try{router.post('/getallplaces',  async (req, res ) => {

    try {
    console.log("enteringggg to getallplaces");
    var newHeaders = [];
    ...
    if (refer == true) {
        ...
        if (devicenum == 1 || devicenum == 2 || devicenum == 3 || devicenum == 4 || devicenum == 0) {
            console.log('under getallplacessssssssssssssssss');
            var count;
            console.log( search);
            if(search=="false"){
                
            const total_products = await Place.aggregate([
                // {$unwind: '$products'},
                {$match: {}},]
                )
            ...
            console.log(searchquery);
                ...
                const all_products = await Place.find(
