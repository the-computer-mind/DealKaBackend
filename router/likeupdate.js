...
router.post('/likeupdate' , async (req, res) => {
    console.log('connection of likeupdate done');
    ...
    //validating jwttoken
    try {
      console.log("enteringggg to likeupdatelikeupdatevv");
      var newHeaders = [];
              ...
              if("size_v<487241590"=="size_v<487241590") { //in bytes
                ...
                Video.aggregate([
                    ...
                    if (userExist1.length==0) {
                        ...
                        console.log("l1");
                        const videoh2 = await Video.findOne({VideoId: videoid});
                        if(videoh2.viewers==null) {
                        ...
                        if(jasonn.like='like') {
                            const mdone = await Video.updateOne({ VideoId: videoid },
                                { "$set": { "no_views": videoh.no_views+1 ,"video_like": videoh.video_like+1 } },{upsert:false,strict:false});
                            console.log(mdone);
                        }else if(jasonn.like='dislike') {
                            const mdone = await Video.updateOne({ VideoId: videoid },
                                { "$set": { "no_views": videoh.no_views+1 ,"video_dislike": videoh.video_dislike+1 } },{upsert:false,strict:false});
                            console.log(mdone);
                        }
